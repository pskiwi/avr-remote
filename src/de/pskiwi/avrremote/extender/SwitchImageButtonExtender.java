...
public final class SwitchImageButtonExtender {
	...
	public static void extend(final ImageView button, final Context ctx,
			ZoneState zone, final Class<? extends AbstractSwitch> s,
			final int onRessource, final int offRessource) {
		...
		zone.setListener(new IStateListener<AbstractSwitch>() {
			public void changedState(AbstractSwitch s) {
				button.setImageResource(!s.isOn() ? onRessource : offRessource);
			}
		}, s);
